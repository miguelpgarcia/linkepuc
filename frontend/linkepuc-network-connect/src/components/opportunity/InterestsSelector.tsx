...
interface InterestsSelectorProps {
  ...
  existingInterests?: string[]; // Optional: array of interest names to pre-select
}

export function InterestsSelector({ selectedInterests, onInterestsChange, existingInterests }: InterestsSelectorProps) {
  ...
  // Initialize selected interests based on existingInterests names
  useEffect(() => {
    if (existingInterests && existingInterests.length > 0 && availableInterests.length > 0 && !initialized) {
      console.log("Initializing selected interests from existing:", existingInterests);
      console.log("Available interests:", availableInterests);
      
      const interestIds = existingInterests
        .map(interestName => {
          const interest = availableInterests.find(i => i.nome === interestName);
          console.log(`Looking for "${interestName}", found:`, interest);
          return interest ? String(interest.id) : null;
        })
        .filter(id => id !== null) as string[];
      
      console.log("Mapped interest IDs:", interestIds);
      
      if (interestIds.length > 0) {
        onInterestsChange(interestIds);
      }
      
      setInitialized(true);
    }
  }, [existingInterests, availableInterests, initialized, onInterestsChange]);
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Selecione os interesses que melhor descrevem esta oportunidade.
      </p>
    </div>
  );
}
